...
public class HumbleComputer {
    ...
    HumbleLongWord StackPointer = new HumbleLongWord();
    HumbleLongWord FourBytes = new HumbleLongWord();
    HumbleLongWord CallLongValue = new HumbleLongWord();
    int CallIntValue;
    ...
    public void execute() {
        ...
        if(!opcode[0].getValue() && opcode[1].getValue() && opcode[2].getValue() && !opcode[3].getValue()) { //if opcode = 0110
            STACK();
        }
    ...
    /**
     * This will read the bits of current instruction and call the correct command.
     */
    public void STACK(){
        /**
         * Gets the address from the last 4 bits of the 16 bit register. Uses that to write to the ending of the memory
         * We use a new method stackwrite to write from the 8196th bit up to the StackPointer value. We subtract the pointer
         * so that we know to read more bits of memory.
         */
        if(!currentInstruction.getBit(27).getValue() && !currentInstruction.getBit(26).getValue()) { //00 - push
            SelectedRegister.humblelong[0] = currentInstruction.getBit(16) ;
            SelectedRegister.humblelong[1] = currentInstruction.getBit(17) ;
            SelectedRegister.humblelong[2] = currentInstruction.getBit(18) ;
            SelectedRegister.humblelong[3] = currentInstruction.getBit(19) ;
            ...
            RegisterNumber = SelectedRegister.getSigned();
            member.stackwrite(StackPointer,register[RegisterNumber]);
            StackPointer = HumbleAdder.subtract(StackPointer,FourBytes);
        }
        /**
         * Gets the address from the last 4 bits of the 16 bit register. Uses that to write to the ending of the memory
         * We use a new method stackwrite to write from the 8196th bit up to the StackPointer value. We add to the pointer
         * so that we know to read less bits of memory. Pop will always pop the top, we do that with a longword zero since
         * the equivalent of popping is replacing the value with zero.
         */
        if(!currentInstruction.getBit(27).getValue() && currentInstruction.getBit(26).getValue()) { //01 - pop
            SelectedRegister.humblelong[0] = currentInstruction.getBit(16) ;
            SelectedRegister.humblelong[1] = currentInstruction.getBit(17) ;
            SelectedRegister.humblelong[2] = currentInstruction.getBit(18) ;
            SelectedRegister.humblelong[3] = currentInstruction.getBit(19) ;

            HumbleLongWord zero = new HumbleLongWord();
            member.stackwrite(StackPointer,zero);
            StackPointer = HumbleAdder.add(StackPointer,FourBytes);
        }
        /**
         * Takes the last 10 bits, converts to an int value. Reads from memory to know what to write,
         * then writes using what was gotten from reading.
         */
        if(currentInstruction.getBit(27).getValue() && !currentInstruction.getBit(26).getValue()) {
            CallLongValue.humblelong[9] = currentInstruction.getBit(25);
            CallLongValue.humblelong[8] = currentInstruction.getBit(24);
            CallLongValue.humblelong[7] = currentInstruction.getBit(23);
            CallLongValue.humblelong[6] = currentInstruction.getBit(22);
            CallLongValue.humblelong[5] = currentInstruction.getBit(21);
            CallLongValue.humblelong[4] = currentInstruction.getBit(20);
            CallLongValue.humblelong[3] = currentInstruction.getBit(19);
            CallLongValue.humblelong[2] = currentInstruction.getBit(18);
            CallLongValue.humblelong[1] = currentInstruction.getBit(17);
            CallLongValue.humblelong[0] = currentInstruction.getBit(16);

            CallIntValue = CallLongValue.getSigned();

            HumbleLongWord CallValue = new HumbleLongWord();
            CallValue = member.read(CallLongValue);
            member.stackwrite(StackPointer,CallValue);
        }
        if(currentInstruction.getBit(27).getValue() && currentInstruction.getBit(26).getValue()) {
            System.out.println(member.StackToString());
        }
    }
